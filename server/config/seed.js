    ...
    .then((data) => {
      if (!data) {
        return ApplicationSetting.create({
          ...
          cdnServer: {
            accessKeyId: 'AKIAIC7GDGVLSBU2LD6A',
            secretAccessKey: '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',
            region: 'us-west-1',
            bucketName: 'staging.viewinfocus.com',
            serverUrl: 'https://s3-us-west-1.amazonaws.com/staging.viewinfocus.com',
          },
          credit700: {
    ...
    .then((data) => {
      if (!data || data.length === 0) {
        return Role.create({
          ...
          name: 'vifManager',
          nameSlug: 'VIF-Manager',
          active: true
        }, {
          name: 'vifAssociate',
          nameSlug: 'VIF-Associate',
          active: true
        }, {
          name: 'dealerAdmin',
    ...
    .then((data) => {
      if (!data || data.length === 0) {
        return Position.create({
          ...
          name: 'financeManager',
          nameSlug: 'Finance Manager',
          role: _.findLast(roles, {name: 'dealerManager'}),
          active: true
        }, {
          name: 'bdcManager',
          nameSlug: 'BDC Manager',
          role: _.findLast(roles, {name: 'dealerManager'}),
          active: true
        }, {
          name: 'serviceManager',
          ...
          nameSlug: 'Accessory Manager',
          role: _.findLast(roles, {name: 'dealerManager'}),
          active: true
        }, {
          name: 'teamLeader',
          nameSlug: 'Team Leader',
    ...
    .then((data) => {
      if (!data || data.length === 0) {
        return User.create({
          ...
          roles: _.find(prevData.roles, {name: 'vifAdmin'}),
          name: {first: 'VIF', last: 'Admin'},
