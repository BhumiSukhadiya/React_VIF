...
export function isAuthenticated() {
  ...
  // Validate jwt
    .use(function (req, res, next) {
      ...
      // return error if no authorization header passed
...
export function hasRole(roleRequired) {
  ...
  return compose()
    ...
    .use(function meetsRequirements(req, res, next) {
      if (_.findIndex(req.user.roles, {name: roleRequired}) >= 0) {
        return next();
      } else if (req.user.roles) {
        for (let i = 0; i < req.user.roles.length; i++) {
          let role = req.user.roles[i];
          if (constants.ROLES.indexOf(role.name) >= constants.ROLES.indexOf(roleRequired)) {
            return next();
          }
        }
      }
