...
describe('Position API:', function() {
  ...
  describe('POST /api/positions', function() {
    beforeEach(function(done) {
      request(app)
        ...
        .send({
          name: 'New Position',
          nameSlug: 'This is the brand new position!!!'
        })
        .expect(201)
        .expect('Content-Type', /json/)
        .end((err, res) => {
    ...
    it('should respond with the newly created position', function() {
      ...
      expect(newPosition.nameSlug).to.equal('This is the brand new position!!!');
    });
  });
  ...
  describe('GET /api/positions/:id', function() {
    ...
    beforeEach(function(done) {
      request(app)
        .get(`/api/positions/${newPosition._id}`)
        .set('authorization', `Bearer ${global.token}`)
        .expect(200)
        .expect('Content-Type', /json/)
        .end((err, res) => {
    ...
    it('should respond with the requested position', function() {
      ...
      expect(position.nameSlug).to.equal('This is the brand new position!!!');
    });
  });
  ...
  describe('PUT /api/positions/:id', function() {
    ...
    beforeEach(function(done) {
      request(app)
        .put(`/api/positions/${newPosition._id}`)
        .set('authorization', `Bearer ${global.token}`)
        .send({
          name: 'Updated Position',
          nameSlug: 'This is the updated position!!!'
        })
        .expect(200)
        .expect('Content-Type', /json/)
    ...
    it('should respond with the updated position', function() {
      ...
      expect(updatedPosition.nameSlug).to.equal('This is the updated position!!!');
    });
    ...
    it('should respond with the updated position on a subsequent GET', function(done) {
      request(app)
        ...
        .end((err, res) => {
          ...
          expect(position.nameSlug).to.equal('This is the updated position!!!');
  ...
  describe('PATCH /api/positions/:id', function() {
    ...
    beforeEach(function(done) {
      request(app)
        .patch(`/api/positions/${newPosition._id}`)
        .set('authorization', `Bearer ${global.token}`)
        .send([
          { op: 'replace', path: '/name', value: 'Patched Position' },
          { op: 'replace', path: '/nameSlug', value: 'This is the patched position!!!' }
        ])
        .expect(200)
        .expect('Content-Type', /json/)
        .end(function(err, res) {
    ...
    it('should respond with the patched position', function() {
      ...
      expect(patchedPosition.nameSlug).to.equal('This is the patched position!!!');
    });
  });
  ...
  describe('DELETE /api/positions/:id', function() {
    it('should respond with 204 on successful removal', function(done) {
      request(app)
        .delete(`/api/positions/${newPosition._id}`)
        .set('authorization', `Bearer ${global.token}`)
        .expect(204)
        .end(err => {
          if(err) {
